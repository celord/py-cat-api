...
class ImageShort:
    def __init__(self, id: int, url: str, categories: List[Category] = None,
                 breeds: List[Breed] = None, data: bytes = bytes(), **kwargs):
        self.id = id
        self.url = url
        self.categories = [] if not categories else [
    ...
    def save_to(self, path: str = './', file_name: str = ''):
        ...
        try:
            save_file_name = file_name if file_name else self.url.split(
                '/')[-1]
